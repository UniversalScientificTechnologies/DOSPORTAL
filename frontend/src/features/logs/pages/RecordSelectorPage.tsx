...
export const RecordSelectorPage = () => {
  ...
  const allRecords: SpectralRecord[] = (recordsQuery.data?.data?.results ?? []).map((r) => ({
    id: r.id,
    name: r.name ?? '',
    created: r.created ?? '',
    author: r.author?.username ?? null,
    owner: r.owner?.name ?? null,
    raw_file_id: r.raw_file?.id ?? null,
    artifacts_count: Number(r.artifacts_count),
  ...
  const error = recordsQuery.isError ? ((recordsQuery.error as Error)?.message ?? 'Unknown error') : null
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: theme.spacing.md }}>
            ...
            <Button variant='secondary'
              ...
              title='If your files are not uploaded yet, upload them first.'
            >
              Upload Files
            </Button>
